...
class TodoTaskCategory(models.Model):
    _name = 'clv_todo.task.category'

    name = fields.Char('Category', required=True, size=128, translate=True)
    parent_id = fields.Many2one('clv_todo.task.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('clv_todo.task.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(TodoTaskCategory, self).name_get()
